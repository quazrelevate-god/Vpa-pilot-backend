...
export default function CrowdLoginPage() {
  ...
  const inputCls =
    "h-12 rounded-xl text-base transition-shadow focus-visible:border-[#1E40AF] focus-visible:ring-2 focus-visible:ring-[#1E40AF]/30";
      ...
      <motion.div variants={container} initial="hidden" animate="show"
        ...
        <motion.div variants={item} className="flex items-center gap-3">
          ...
          <div className="leading-tight">
            <div className="text-[1.3rem] font-black tracking-tight text-slate-900">{t("Crowd", "கூட்ட")}</div>
            <div className="-mt-1 text-[1.3rem] font-black tracking-tight text-slate-900">{t("Management", "மேலாண்மை")}</div>
          </div>
        ...
        <motion.div variants={item} className="relative mt-5">
          <motion.div
            ...
            <HeroIllustration className="w-full drop-shadow-[0_18px_30px_-18px_rgba(33,57,91,0.35)]" />
          </motion.div>
        </motion.div>

        {/* Login card */}
        <motion.div variants={item}
          className="mt-5 rounded-3xl border border-white/70 bg-white/80 p-6 shadow-[0_18px_46px_-24px_rgba(28,30,41,0.35)] backdrop-blur-xl">
          <h1 className="text-lg font-extrabold text-slate-900">{t("Floor Operator App", "தள ஆபரேட்டர் செயலி")}</h1>
          <p className="mt-1 text-sm leading-relaxed text-slate-500">
            {t("Sign in to manage today's visitors.", "இன்றைய வருகையாளர்களை நிர்வகிக்க உள்நுழையவும்.")}
          </p>
