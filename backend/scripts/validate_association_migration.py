"""
Validate the petition→association migration BEFORE anything is deleted.

For every association that was migrated (association_submissions.
source_appointment_id is set) this checks the copy is complete and
self-contained:

  • the source appointment still exists and is in the target category;
  • no two associations claim the same source appointment;
  • the row has >= 1 document AND every document lives under associations/
    (i.e. copied into its own MinIO namespace, NOT still pointing at the
    petition's soon-to-be-deleted attachment key);
  • the primary document object actually exists in MinIO;
  • extraction_json is present.

Exit code 0 = every migrated association is safe to delete its petition from.
Exit code 1 = at least one FAIL — delete_migrated_association_petitions.py must
not run.

    cd backend
    ./env/bin/python scripts/validate_association_migration.py
"""
...
async def validate(*, category: str) -> int:
    ...
    async with AsyncSessionLocal() as db:
        ...
        dupes = {aid: n for aid, n in Counter(
            a.source_appointment_id for a in migrated
        ).items() if n > 1}

        print(f"[validate] {len(migrated)} migrated association(s); "
              f"{len(appt_ids)} petition(s) still in category={category!r}\n")

        for a in migrated:
            problems = []
            src = a.source_appointment_id

            if dupes.get(src):
                problems.append(f"source appointment {src} claimed by {dupes[src]} associations")
            ...
            if not docs:
                ...
                offenders = [d.get("storage_url", "") for d in docs
                             if not str(d.get("storage_url", "")).startswith("associations/")]
                if offenders:
                    problems.append(f"document(s) not copied into associations/ namespace: {offenders}")
                else:
            ...
            if not a.extraction_json:
                problems.append("empty extraction_json")

            if problems:
                fails += 1
                print(f"  FAIL  association {a.id} (assoc={a.association_name!r}, src_appt={src}):")
                for pr in problems:
                    print(f"          - {pr}")
            else:
                ok += 1
    ...
    print(f"  FAIL : {fails}")
    if fails:
        print("  → NOT safe to delete. Fix the failing associations (re-run migrate) first.")
        return 1
    print("  → every migrated petition has a complete, self-contained association. Safe to delete.")
    return 0


def main() -> None:
    p = argparse.ArgumentParser(description="Validate the petition→association migration before deletion.")
