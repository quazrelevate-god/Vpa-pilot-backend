"""
Simple signed-cookie session auth for the Events (invitation calendar) PWA.
Separate cookie name from the dashboard/crowd auth so sessions are independent.
...
def create_events_cookie(response, username: str):
...
def clear_events_cookie(response):
    response.delete_cookie(_COOKIE_NAME)


def get_events_user(request: Request) -> Optional[str]:
    """Return the signed-in events username, or None. Never raises."""
...
def require_events_api(request: Request) -> str:
